/** @odoo-module **/
    ...
    publicWidget.registry.Delivery_date = publicWidget.Widget.extend({
        ...
        _onChangeDeliveryDate: function(ev) {
           ...
           rpc('/delivery_date_schedule', {'date': this.input_date}).then(function(data) {
        ...
        _onClickConfirm: function(ev) {
            var self = this;
            if (!this.error_value) {
                   self.el.querySelector("#error_for_date").innerHTML = "please provide the delivery date"
                return false;
            } else if (this.error_value == 2) {
                  self.el.querySelector("#error_for_date").innerHTML = "please provide the delivery date between " + self.min_date + " and " + self.max_date
                return false;
            } else {
                rpc('/confirm_delivery_date_schedule', {'id': ev.currentTarget.getAttribute("value"),
